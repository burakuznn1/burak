...
const App: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-[100dvh] bg-[#F8FAFC]">
      
      {/* Mobile Top Bar (iPhone Safe Area Fixed) */}
      <header className="lg:hidden fixed top-0 left-0 right-0 bg-[#001E3C] text-white p-4 flex justify-between items-center z-[60] shadow-lg border-b border-white/10 safe-top">
        <div className="flex items-center gap-2">
          <Award size={24} />
          <span className="font-black tracking-tighter text-lg uppercase">TÜRKWEST</span>
        </div>
        <div className="flex items-center gap-4">
           {!isAdminAuthenticated && (
             <button onClick={() => setShowLoginModal(true)} className="p-2 bg-white/10 rounded-xl transition-transform active:scale-95">
               <Lock size={20} />
             </button>
           )}
           {isAdminAuthenticated && (
      ...
      <aside className="hidden lg:flex w-72 bg-[#001E3C] text-white flex-col fixed inset-y-0 z-50 shadow-2xl">
        ...
        <div className="p-6 border-t border-white/5">
           {isAdminAuthenticated ? (
             <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 py-3 bg-red-500/10 text-red-400 rounded-xl text-xs font-bold hover:bg-red-500/20 transition-all"><LogOut size={16}/> Çıkış</button>
           ) : (
             <button onClick={() => setShowLoginModal(true)} className="w-full py-3 bg-white/10 rounded-xl text-xs font-bold flex items-center justify-center gap-2 hover:bg-white/20 transition-all"><Lock size={14}/> Yönetici Paneli</button>
           )}
        </div>
      </aside>
      ...
      <main className="flex-1 lg:ml-72 p-4 lg:p-10 pb-32 pt-24 lg:pt-10">
        {!selectedPropertyId && !isAdminAuthenticated && (
          <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4 animate-in zoom-in-95 duration-500">
             <div className="w-20 h-20 bg-white rounded-[2rem] shadow-xl flex items-center justify-center mb-8 border border-slate-50">
                <Award className="text-[#001E3C]" size={40} />
             </div>
             <h1 className="text-4xl lg:text-5xl font-black text-[#001E3C] mb-4 tracking-tight">Varlık Raporu</h1>
             <p className="text-slate-500 mb-10 max-w-xs mx-auto font-medium">Size özel mülk performans analizini görüntülemek için kodunuzu girin.</p>
             <form onSubmit={(e) => { 
               e.preventDefault(); 
               ...
               else { alert('Geçersiz mülk kodu.'); } 
             }} className="w-full max-w-sm space-y-4">
               <input type="text" placeholder="Örn: west-101" value={clientCodeInput} onChange={e => setClientCodeInput(e.target.value)} className="w-full px-6 py-5 bg-white border-2 border-slate-100 rounded-[2rem] text-center text-2xl font-black uppercase outline-none focus:border-[#001E3C] text-[#001E3C] shadow-sm placeholder:text-slate-200" />
               <button className="w-full py-5 bg-[#001E3C] text-white rounded-[2rem] font-black text-lg shadow-2xl hover:bg-slate-800 transition-all active:scale-95">Raporu Görüntüle</button>
             </form>
          </div>
        )}
             ...
             <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <button onClick={() => {
                   ...
                   const newProp: Property = { 
                     id, title: 'Yeni Portföy', location: 'Rize', image: 'https://images.unsplash.com/photo-1560518883-ce09059eeffa?auto=format&fit=crop&w=1000&q=80',
                   ...
                   setActiveTab('edit');
                }} className="w-full sm:w-auto px-8 py-4 bg-[#001E3C] text-white rounded-2xl font-black flex items-center justify-center gap-2 shadow-xl hover:bg-slate-800 transition-all active:scale-95"><Plus size={24}/> Yeni İlan Ekle</button>
             </div>
             <div className="relative group">
                <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[#001E3C] transition-colors" size={20}/>
                <input type="text" placeholder="Portföy ara..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-16 pr-8 py-5 bg-white border border-slate-100 rounded-[2rem] outline-none text-[#001E3C] font-bold shadow-sm focus:border-blue-200 transition-all" />
             </div>
             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredProperties.map(p => (
                  <div key={p.id} className="bg-white rounded-[2.5rem] overflow-hidden border border-slate-100 shadow-sm hover:shadow-2xl transition-all group cursor-pointer relative" onClick={() => { setSelectedPropertyId(p.id); setActiveTab('dashboard'); }}>
                     <div className="h-56 relative overflow-hidden">
                       <img src={p.image} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" />
                       <div className="absolute top-4 left-4 px-3 py-1 bg-[#001E3C] text-white rounded-lg text-[10px] font-black shadow-lg">{p.id}</div>
                     </div>
                     <div className="p-8 space-y-6">
                        <div className="flex justify-between items-start gap-4">
                           <div className="space-y-1">
                             <h4 className="font-black text-xl text-[#001E3C] leading-tight line-clamp-1">{p.title}</h4>
                             <p className="text-xs font-black text-slate-300 flex items-center gap-1 uppercase tracking-widest"><MapPin size={12}/> {p.location || 'Rize'}</p>
                           </div>
                           <div className="flex items-center gap-1 text-blue-500 shrink-0 bg-blue-50 px-3 py-1 rounded-full"><Eye size={14}/> <span className="text-xs font-black">{p.viewCountByClient || 0}</span></div>
                        </div>
                        <div className="flex gap-3">
                           <button onClick={(e) => { e.stopPropagation(); setSelectedPropertyId(p.id); setActiveTab('dashboard'); }} className="flex-1 py-4 bg-[#001E3C] text-white rounded-2xl text-xs font-black shadow-lg hover:shadow-2xl transition-all">Raporu Gör</button>
                           <button onClick={(e) => { e.stopPropagation(); setSelectedPropertyId(p.id); setActiveTab('edit'); }} className="p-4 bg-slate-50 text-slate-300 rounded-2xl hover:text-[#001E3C] hover:bg-blue-50 transition-all"><Edit3 size={20}/></button>
                        </div>
                     </div>
                  </div>
                ))}
          ...
          <div className="max-w-6xl mx-auto space-y-10 animate-in fade-in duration-700">
             <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
                <div className="space-y-2">
                   ...
                   <p className="flex items-center gap-2 text-slate-500 font-bold text-sm"><MapPin size={16} className="text-red-400"/> {currentProperty.location || 'Rize'}</p>
                </div>
                <div className="flex gap-4 w-full lg:w-auto">
                   {isAdminAuthenticated && <button onClick={() => setActiveTab('edit')} className="p-5 bg-white border border-slate-100 rounded-3xl shadow-sm hover:shadow-xl transition-all"><Edit3 size={28} className="text-[#001E3C]"/></button>}
                   <button onClick={handleGenerateAISummary} disabled={isGenerating} className="flex-1 lg:flex-none px-10 py-5 bg-[#001E3C] text-white rounded-3xl font-black shadow-2xl flex items-center justify-center gap-3 hover:bg-slate-800 transition-all active:scale-95 disabled:opacity-50">
             ...
             {aiSummary && (
               <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-blue-50 animate-in slide-in-from-bottom-5">
                 <h4 className="text-xl font-black text-[#001E3C] mb-6 flex items-center gap-3 border-b pb-4"><Sparkles size={24} className="text-amber-500"/> Danışman Analizi</h4>
                 <p className="whitespace-pre-line text-[#001E3C] leading-relaxed font-bold text-lg italic">"{aiSummary}"</p>
               </div>
             )}

             <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                <DashboardStat label="İlan Görülme" value={latestStats.views} prevValue={previousStats?.views} icon={<Eye size={24}/>} color="blue" />
                <DashboardStat label="Favoriye Ekleme" value={latestStats.favorites} prevValue={previousStats?.favorites} icon={<Heart size={24}/>} color="red" />
                <DashboardStat label="Mesaj & Çağrı" value={latestStats.messages + latestStats.calls} prevValue={previousStats ? (previousStats.messages + previousStats.calls) : undefined} icon={<MessageSquare size={24}/>} color="indigo" />
                <DashboardStat label="Yer Gösterme" value={latestStats.visits} prevValue={previousStats?.visits} icon={<Navigation size={24}/>} color="emerald" />
             </div>

             <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-8">
                   <h4 className="text-xl font-black text-[#001E3C] border-b pb-4">Güncel Teklif Akışı</h4>
                   <div className="space-y-4 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                      {(!currentProperty.offers || currentProperty.offers.length === 0) ? (
                        <div className="text-center py-20 text-slate-200 italic font-black text-2xl">Teklif Bekleniyor...</div>
                      ) : (
                        currentProperty.offers.map(offer => (
                          <div key={offer.id} className="flex items-center justify-between p-6 bg-slate-50/50 rounded-2xl border border-slate-100">
                             <div className="flex items-center gap-5">
                               <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-slate-200 shadow-inner border border-slate-50"><User size={24}/></div>
                               <div>
                                 <p className="font-black text-lg text-[#001E3C]">{offer.bidder.substring(0,2)}*****</p>
                                 <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{offer.date} • {offer.status}</p>
                               </div>
                             </div>
                             <p className="font-black text-xl text-[#001E3C]">₺{offer.amount.toLocaleString()}</p>
                          </div>
                        ))
                      )}
                   </div>
                ...
                <div className="bg-[#001E3C] p-12 rounded-[2.5rem] text-white flex flex-col justify-between shadow-2xl relative overflow-hidden group">
                   <div className="space-y-10 relative z-10">
                      <div className="flex items-center gap-4"><MessageCircle size={40} className="text-blue-400"/><h4 className="text-2xl font-black tracking-tight">Danışman Notu</h4></div>
                      <p className="text-xl font-medium italic text-white/80 leading-relaxed">"{currentProperty.agentNotes || 'Satış süreciniz profesyonel ekibimizce takip edilmektedir.'}"</p>
                   </div>
                   <div className="mt-16 pt-10 border-t border-white/10 flex items-center justify-between gap-6 relative z-10">
                      <div className="flex items-center gap-5">
                         <div className="w-14 h-14 bg-white/10 rounded-[1.2rem] flex items-center justify-center text-blue-400">
                            <UserCheck size={32}/>
                         </div>
                         <div>
                            <p className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1">Portföy Yöneticisi</p>
                            <p className="text-xl font-black tracking-tighter">{currentProperty.agentName || 'TÜRKWEST Danışman'}</p>
                         </div>
                      </div>
                      {currentProperty.agentPhone && (
                        <a href={`tel:${currentProperty.agentPhone}`} className="w-14 h-14 bg-blue-500 text-white rounded-2xl flex items-center justify-center hover:scale-110 transition-transform shadow-xl">
                           <Phone size={28}/>
                        </a>
                      )}
                   </div>
                   <div className="absolute top-0 right-0 p-8 opacity-5 rotate-12 group-hover:scale-110 transition-transform duration-1000"><Award size={150}/></div>
                </div>
             </div>

             {!isAdminAuthenticated && (
                <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-8 max-w-2xl mx-auto text-center">
                  <h4 className="text-3xl font-black text-[#001E3C] tracking-tight">Danışmana Mesaj Gönder</h4>
                  {showFeedbackSuccess ? ( 
                    <div className="bg-emerald-50 text-emerald-600 p-12 rounded-[2.5rem] font-black text-2xl animate-in zoom-in-95">Talebiniz başarıyla iletildi.</div> 
                  ) : (
                    <div className="space-y-6">
                      <input type="text" placeholder="Fiyat Revize Talebiniz (₺)" value={clientRequestedPriceInput} onChange={e => setClientRequestedPriceInput(e.target.value.replace(/[^0-9]/g, ''))} className="w-full px-8 py-5 bg-slate-50 border border-slate-200 rounded-3xl outline-none font-black text-xl text-[#001E3C]" />
                      <textarea value={clientNoteInput} onChange={e => setClientNoteInput(e.target.value)} placeholder="Talebiniz..." className="w-full h-40 p-8 bg-slate-50 border border-slate-200 rounded-[2rem] outline-none font-bold text-lg text-[#001E3C] resize-none" />
                      <button onClick={handleAddFeedback} className="w-full py-6 bg-[#001E3C] text-white rounded-[2rem] font-black text-xl shadow-2xl flex items-center justify-center gap-4 transition-all active:scale-95 hover:bg-slate-800"><Send size={24}/> TALEBİ İLET</button>
                    </div>
                  )}
                </div>
             )}
        ...
        {/* LocalStorage crash prevention for other tabs... */}
        {activeTab === 'portfolioStats' && isAdminAuthenticated && (
          <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
             <h2 className="text-3xl font-black text-[#001E3C]">Genel Veriler</h2>
             <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <DashboardStat label="Toplam Portföy" value={properties.length} icon={<Home size={22}/>} color="blue" />
                <DashboardStat label="Teklif Adedi" value={properties.reduce((acc, p) => acc + (p.offers?.length || 0), 0)} icon={<Wallet size={22}/>} color="emerald" />
                <DashboardStat label="Rapor Görülme" value={properties.reduce((acc, p) => acc + (p.viewCountByClient || 0), 0)} icon={<Eye size={22}/>} color="indigo" />
             </div>
          </div>
        )}
      </main>
      ...
      {isAdminAuthenticated && (
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-[#001E3C] text-white/40 border-t border-white/5 px-6 py-4 flex justify-around items-center z-[60] backdrop-blur-xl safe-bottom">
           ...
           <MobileNavItem icon={<Bell size={22}/>} label="Talepler" active={activeTab === 'notifications'} onClick={() => setActiveTab('notifications')} badge={allFeedbacks.length} />
        </nav>
      )}

      {/* PIN Modal (iPhone Safe) */}
      {showLoginModal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-[#001E3C]/95 backdrop-blur-xl p-4">
           <div className="bg-white w-full max-w-sm rounded-[3.5rem] p-12 text-center shadow-2xl animate-in zoom-in-95 duration-300">
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                 ...
                 <button type="button" onClick={() => setShowLoginModal(false)} className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-6 hover:text-red-500 transition-colors">Vazgeç</button>
              </form>
           </div>
        </div>
      )}
...
const NavItem = ({ icon, label, active, onClick, badge }: any) => (
  <button onClick={onClick} className={`w-full flex items-center justify-between px-6 py-4 rounded-2xl transition-all duration-300 ${active ? 'bg-white/10 text-white font-black shadow-lg' : 'text-white/30 hover:text-white hover:bg-white/5'}`}>
    <div className="flex items-center gap-4"><span>{icon}</span><span className="text-[13px] tracking-tight">{label}</span></div>
    {(badge || 0) > 0 && <span className="w-6 h-6 bg-red-500 text-white text-[10px] font-black rounded-full flex items-center justify-center animate-pulse border-2 border-[#001E3C]">{badge}</span>}
  </button>
);
...
const MobileNavItem = ({ icon, label, active, onClick, badge }: any) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-1.5 transition-all relative ${active ? 'text-white scale-110' : 'text-white/30'}`}>
    <div className="relative">
      {icon}
      {(badge || 0) > 0 && <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-red-500 text-white text-[8px] font-black rounded-full flex items-center justify-center ring-2 ring-[#001E3C]">{badge}</span>}
    </div>
...
const DashboardStat = ({ label, value, prevValue, icon, color }: any) => {
  ...
  return (
    <div className="bg-white p-7 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-4 hover:shadow-xl transition-all duration-500 group">
       <div className={`w-14 h-14 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform ${styles[color]}`}>{icon}</div>
       <div>
          <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">{label}</p>
          <div className="flex items-baseline gap-2 mt-1">
            <h4 className="text-2xl font-black text-[#001E3C] tracking-tighter">{valNum.toLocaleString()}</h4>
            {diff !== null && diff !== 0 && (
              <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${diff > 0 ? 'bg-emerald-50 text-emerald-500' : 'bg-red-50 text-red-500'}`}>
                {diff > 0 ? '+' : ''}{diff}
              </span>
            )}
          </div>
...
const MarketMetric = ({ label, value, icon }: any) => (
  <div className="bg-white p-7 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center gap-5 hover:shadow-md transition-all">
     <div className="w-14 h-14 bg-slate-50 text-slate-300 rounded-2xl flex items-center justify-center shrink-0">{icon}</div>
     <div className="flex-1 min-w-0"><p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mb-1">{label}</p><h4 className="text-xl font-black text-[#001E3C] truncate">{value}</h4></div>
  </div>
);
...
const AdminInput = ({ label, value, onChange, type = "text" }: any) => (
  <div className="space-y-1 w-full">
    ...
    }} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-black outline-none focus:border-[#001E3C] text-[#001E3C] shadow-inner transition-all" />
  </div>
);
